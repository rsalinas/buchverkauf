...
public class AbteilungFrame {

	private final Shell sh;

	public AbteilungFrame(final DsvBuecher gui, Shell pshell,
			final Abteilung data) {
		sh = new Shell(pshell, SWT.RESIZE | SWT.CLOSE | SWT.APPLICATION_MODAL);
		sh.setText("Abteilungsverwaltung");
				...
				BusyIndicator.showWhile(sh.getDisplay(), new Runnable() {
					...
					public void run() {
						try {
							...
							if (data.isNew())
								gui.ds.abteilungenVw.insertAbteilung(data);
							else
								gui.ds.abteilungenVw.updateAbteilung(data);
