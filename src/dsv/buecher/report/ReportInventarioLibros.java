...
public class ReportInventarioLibros {
	String headers[] = { "Stock", "Rsv", "Vfüg.", "Titel", "Verlag", "ISBN",
			BestelllisteStaticized.klassenCaption };

	public ReportInventarioLibros(DsvBuecher parent) throws SQLException,
			...
			for (String land : new String[] { "e", "d", "l" }) {
...

				{
					...
					final Phrase p3 = new Phrase(
							"Inventar (zum unten angegebenen Zeitpunkt)");
					final Phrase p4 = new Phrase("Bücher aus: "
							+ Verlag.hm.get(land), f14);
					f14.setStyle(Font.BOLD);
					final PdfPCell c3 = new PdfPCell(p3);
					final PdfPCell c5 = new PdfPCell(p4);
				...
				PdfPTable table;
				table = new PdfPTable(new float[] { 1.5f, 1.5f, 1.5f, 8, 6, 4,
						6 });
				...
				for (String verlagName : vArray)
					for (Buch b : Buch.sort(buecherNachVerlag.get(verlagName))) {
						...
						{
							final PdfPCell c12 = new PdfPCell(new Paragraph(
									b.stock + " "));
						...
						{
							int disponibles = b.stock - b.reservations;

							String t0 = disponibles > 0 ? disponibles + " "
									: "--";
							final PdfPCell c12 = new PdfPCell(new Paragraph(t0));
							c12.setHorizontalAlignment(Element.ALIGN_RIGHT);
							c12.setRightIndent(10);
							table.addCell(c12);
						}
				...
				document.add(table);
				document.add(new Paragraph(
						"Stock = Momentan vorhandene Bücheranzahl"));
				document.add(new Paragraph(
						"Rsv = Reservierte und noch nicht verkaufte Bücher"));
				document
						.add(new Paragraph(
								"Vfüg. = Übrig gebliebene Bücher (nicht gekauft, obwohl bestellt)"));
