...
import dsv.buecher.BestellungState;
					...
					"klasse,name,vorname"));
		{
			...
			String now = new SimpleDateFormat(dateFormat).format(new Date());
			{
				...
				final PdfPCell c2 = new PdfPCell(new Phrase(MessageFormat
						.format("Schüler die noch nicht bestellt haben {0}", ds
								.getRunningYearName())));
			...
			for (Klasse klasse : klassen) {
				...
				if (!l.isEmpty()) {
					...
					final Phrase p3 = new Phrase(
							"Schüler die noch nicht bestellt haben ("
									+ l.size() + ")", f14);
					f14.setStyle(Font.BOLD);
					final PdfPCell c3 = new PdfPCell(p3);
