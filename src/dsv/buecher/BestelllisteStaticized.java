...
public class BestelllisteStaticized {
	public final static String klassenCaption = "Klassen (aktuelle Zahlen)";
	// Stock
	// Von Schülern bestellt
	// zu bestellen beim Händler

	// "Erklärung: v.Vj.: vom Vorjahr übrig // S.bst.: von Schülern aktuell bestellt //  Bst.H =  zu bestellen beim Händler."));
	String headers[] = { "v.Vj.", "S.bst.", "Bst.H", "", "Titel", "Verlag",
			"ISBN", klassenCaption };

	public BestelllisteStaticized(DsvBuecher parent) throws SQLException,
			MalformedURLException, IOException, DocumentException {
		DsvBuecherDatasource ds = parent.ds;
		File fich = File.createTempFile("Bestellliste-static", ".pdf");
		{
			...
			for (String land : new String[] { "e", "d", "l" }) {
...

				{
					...
					final Phrase p3 = new Phrase(
							"Bestellliste  (bei der Bestellung beim Händler), Bücher aus: "
				...
				PdfPTable table;
				table = new PdfPTable(new float[] { 1.5f, 1.5f, 1.5f, 1f, 8, 6,
						4, 6 });
