...
public class RemesaBrowser {
	private final Table table;
	private ArrayList<Remesa> req;
	private final DsvBuecher gui;

	public RemesaBrowser(final DsvBuecher parent) throws SQLException {
		this.gui = parent;
		this.shell = new Shell(parent.getShell(), SWT.RESIZE | SWT.CLOSE);
		shell.setText("Buchungssätze-Verwaltung");
			...
			new Composite(shell, 0) {
				{
					...
					Button crearButton = RauUtil.createButton(this,
							"Crear remesa", new SelectionAdapter() {
								@Override
								public void widgetSelected(SelectionEvent arg0) {
									try {
										gui.ds.remesaVw.createNewRemesa();
					...
					crearButton.setEnabled(gui.ds.remesaVw
							.newPendingRechnungen());
					final Button bearbeitenButton = RauUtil.createButton(this,
							"Show CSV", new SelectionAdapter() {
								@Override
								public void widgetSelected(SelectionEvent arg0) {
									if (table.getSelectionCount() != 1) {
										shell.getDisplay().beep();
									} else {
										new BuchungsCsvMaker(
												gui,
												(Remesa) table.getSelection()[0]
														.getData());
										gui.modelChanged();
										syncLoadData();
									}
	...
	private void loadData() {
		try {
			req = gui.ds.remesaVw.getRemesas();
